...
#from magenta.models.image_stylization.images_utils import form_image_grid
from tensorflow.examples.tutorials.mnist import input_data

BATCH_SIZE = 50
GRID_ROWS = 5
GRID_COLS = 10
USE_RELU = True # different with '3_autoencoder.py'
...
if __name__ == '__main__':
    main()
    import pandas as pd
    latent_fname = 'latent_relu.csv'
    data = pd.read_csv(latent_fname, delimiter=' ', header=None)
    data.columns = ['latent1', 'latent2', 'label']
    import matplotlib.pyplot as plt

    plt.scatter(data.latent1, data.latent2, c=data.label)
    # plt.scatter(encoder_result[:, 0], encoder_result[:, 1], c=mnist.test.labels)
    plt.colorbar()
    # plt.show()
    print('start save fig')
    plt.savefig('mnist4_0429.png', dpi=500)
